      ...
      {toMap &&
        toMap.map(course => {
          ...
          // If hovering on techer overView
            ...
            const vt = summary20[personHover.name]["VT Courses"].filter(
              c => course === c["Course Code"]
            );
            const ht = summary20[personHover.name]["HT Courses"].filter(
              c => course === c["Course Code"]
            );
          ...
          return (
            <button
              ...
              className={cName}
              data-id={course}
              onClick={e => courseClicked(e.currentTarget.dataset.id)}
